...
class FormsBloc extends Bloc<FormsEvent, FormsState> {
  ...
  Stream<FormsState> _mapOnUpdateValue(OnUpdateValue event) async* {
    ...
    yield* _checkRules(event.fieldName);
  ...
  Stream<FormsState> _checkRules(String excludeFieldName) async* {
    ...
    for (final fieldName in state.fieldStates.keys) {
      ...
      newState[fieldName] = fieldState.copyWith();
    ...
    yield state.copyWith(fieldStates: newState);
  }
    ...
    for (final entry in sortedEntries) {
      ...
      yield* _mapOnFieldError(
        OnFieldError(
          ...
          requestFocus:
              fieldName == sortedEntries.first.key && event.requestFocus,
        ),
      );
    }
  }
  ...
  Stream<FormsState> _mapOnClearAllErrors() async* {
    yield state.copyWith(
      fieldStates: state.fieldStates.map(
        (key, value) => MapEntry(
          key,
          value.copyWith(errorMessage: const Optional.empty()),
        ),
      ),
    );
  }
  ...
  Stream<FormsState> _mapOnUpdateFormValuesFieldEmpty(
      OnRewriteFormValuesFieldEmpty event) async* {
